...
def test_process_and_save_single_file_repairs_flattened_paragraphs_before_write(tmp_path):
    original = (
        '<html><head><title>Chapter</title></head>'
        '<body class="chapter">'
        '<h1>Chapter</h1><p>First.</p><p>Second.</p>'
        '</body></html>'
    )
    prefix, _, suffix = process_body_tag(original, return_parts=True, body_content_only=False)
    translated = '<body class="chapter"><h1>Glava</h1>First translated.\nSecond translated.</body>'
    output_path = tmp_path / "chapter_translated.html"
    parser = ResponseParser(
        worker=SimpleNamespace(use_prettify=False),
        log_callback=lambda _message: None,
    ...
    assert '<p>First translated.</p>' in saved
    assert '<p>Second translated.</p>' in saved

...
* слово, слипшееся из трёх частей, за один проход расклеивается не полностью:
  «дверьдверьИван» → «дверьдверь Иван» → «дверь дверь Иван». В `validation.py`
  починка вызывается один раз за проверку, так что такие тройные склейки
  доживают до следующего запуска. Здесь требуется сходимость.
"""

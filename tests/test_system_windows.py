...
def test_lone_message_next_to_an_unparsed_message_is_left_alone():
    # Соседа-реплику детектор не разбирает: ник в кавычках («Сукуна»), приписка о фото
    # («Star Rail»), канал игрового чата (LOL). Одна рамка посреди такой переписки хуже,
    # чем ни одной.
    for before, line, after in [
        ("«Тунец с майонезом»: Кое-кто готовит десерт на кухне!", "Нобара: Десерт? Кто готовит?",
         "«Тунец с майонезом»: В чате неудобно объяснять, просто дуйте сюда!"),
        ("Стелла: «Юнь Хао всё, не дотянул». (Прикреплено фото: Юнь Хао.jpg)",
         "Вельт: «Спит стоя? Поразительный навык».", "Далее шла переписка между остальными членами экипажа."),
        ("[Всем] AHQ: GG!", "EDG: GG!", "Игроки AHQ благородно напечатали поздравления в общий чат."),
    ]:
        assert find_windows(_chapter("Матч подходил к концу.", before, line, after)) == [], line
    # Повествование с двоеточием, вложение и смайлик соседом-репликой не считаются
    # («Да вы издеваетесь!»: «Ван Ци: [Геолокация]»).
    message = "Ван Ци: «Парни, моя девушка угощает всех!»"
    for before, after in [
        ("Цзян Ци написал в чат: «Ты где?»", "Он убрал телефон."),
        ("В общий чат пришло сообщение от Ван Ци.", "Ван Ци: [Геолокация]"),
...
def test_messages_one_line_of_narration_after_a_chat_continue_it():
    ...
    for html in [
        ...
        _chapter(lead, first, "Он ждал ответа.", reply, "Стелла: «Вы где?». (Прикреплено фото: база.jpg)"),
    ]:
    ...
    # в рамках (Kumo desu ka: «Сяобай: [Обеденный_стол. Jpg]. «…»»).
    series = ["Выверт: «Приговор гарантирован».", "Баланс: «Отставить».", "Выверт: «Понял»."]
    html = _chapter(lead, *series, "Стелла: «Вы где?». (Прикреплено фото: база.jpg)", reply, "Выверт замер.")

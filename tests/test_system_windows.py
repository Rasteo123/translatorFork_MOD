...
# --- «Сын Симург»: вложения в переписке, слова о сообщении после него -------------------


def test_attachment_lines_inside_a_chat_keep_it_whole():
    # «3. Бытие 2»: «Пришло селфи.», «Пришло фото.», «Картинка.» между сообщениями.
    lines = [
        "ТТ: «Мнения расходятся».",
        "Гарри: «Насчет того, девушка ли ты? Или насчет красоты?»",
        "Пришло селфи.",
        "Гарри: «О. Очень красивая. Вау».",
        "ТТ: «Лучше, чем это?»",
        "Картинка.",
        "Гарри: «Черт. Дай мне секунду…»",
    ]
    html = _chapter("Хм. Звучит любопытно.", *lines, "Фотография. Девушка в темной комнате, овальное лицо.")

    assert [(window.kind, window.lines) for window in find_windows(html)] == [("chat", lines)]
    assert not sw._is_chat_spacer("Фотография. Девушка в темной комнате, овальное лицо.")


def test_lone_message_with_the_messaging_word_right_after_it():
    # «47. Восхождение 3»: сообщение на экране, а «удалила сообщение» — следующим абзацем.
    message = "ТТ: «Бласто вшил бомбу в череп. Страховка Выверта»."
    html = _chapter("Она набрала что-то огромным шрифтом в текстовом редакторе.", message,
                    "Прямо у них на глазах она удалила сообщение.")
    assert [(window.kind, window.lines) for window in find_windows(html)] == [("chat", [message])]
    # Слово «телефон» после реплики доводом не считается: так бывает и в сценарной речи.
    html = _chapter("Годжо вышел.", "Фушигуро: «Так я и знал».", "Итадори посмотрел на телефон.")
    assert find_windows(html) == []
    # Реплика после строки — отклик на что угодно, а не довод о переписке.
    html = _chapter("Годжо вышел.", "Фушигуро: «Так я и знал».", "— Кто-нибудь прочитал сообщение? — спросил Итадори.")
    assert find_windows(html) == []


def test_lone_message_next_to_an_unparsed_message_is_left_alone():
    # Соседа-реплику детектор не разбирает: пояснение рассказчика после сообщения
    # («Конан»: «Цзян Лай: «.» (Ничего себе…)»), незнакомая метка перед именем. Одна рамка
    # посреди такой переписки хуже, чем ни одной.
    for before, line, after in [
        ("Цзян Лай: «.» (Ничего себе, вот он какой – богатый наследник!)",
         "Мацуда: «Спит стоя? Поразительный навык».", "Далее шла переписка между остальными."),
        ("[Тренировочный зал] Лю Шэн: Все сюда!", "Сье: GG!", "Игроки напечатали поздравления в общий чат."),
    ]:
        assert find_windows(_chapter("Матч подходил к концу.", before, line, after)) == [], line
    # Повествование с двоеточием, скобки без вложения, реплика без слов и немое «— …»
    # соседом-репликой не считаются.
    message = "Ван Ци: «Парни, моя девушка угощает всех!»"
    for before, after in [
        ("Цзян Ци написал в чат: «Ты где?»", "Он убрал телефон."),
        ("В общий чат пришло сообщение от Ван Ци.", "Ван Ци: [Подарок]"),
        ("В общий чат пришло сообщение от Ван Ци.", "Фэй2–8: «(¬_¬)»"),
        ("В общий чат пришло сообщение от Ван Ци.", "Лу Аньчжи: — …"),
    ]:
        html = _chapter(before, message, after)
        assert [(window.kind, window.lines) for window in find_windows(html)] == [("chat", [message])], after
    # Вложение следом — сообщение той же переписки («Да вы издеваетесь!»).
    html = _chapter("В общий чат пришло сообщение от Ван Ци.", message, "Ван Ци: [Геолокация]", "Кун Лю кликнул.")
    assert [(window.kind, window.lines) for window in find_windows(html)] == [
        ("chat", [message, "Ван Ци: [Геолокация]"])]


def test_messages_one_line_of_narration_after_a_chat_continue_it():
    # «Сын Симург», гл. 14: Выверт переписывается с Балансом, между сообщениями — по
    # строке повествования: «Он ждал ответа…».
    first = "Выверт: «Обвинительный приговор гарантирован»."
    reply = "Баланс: «Отставить. Субъектов необходимо изолировать, но не убивать»."
    question = "Баланс: «Надеюсь, теперь вы понимаете?»"
    pair = ["Выверт: «Понимаю, благодарю вас».", "Баланс: «Принято»."]
    html = _chapter(
        "Этим и был вызван его нынешний диалог по защищенному мобильному телефону.", first,
        "Он ждал ответа с едва сдерживаемым нетерпением.", reply,
        "Выверт замер, пока его мысли лихорадочно работали.",
        "Та вселенная схлопнулась. Он взглянул на телефон.", question,
        "Баланс знал, что Выверт только что использовал свою способность.", *pair,
        "Переписка прервалась.",
    )
    assert [(window.kind, window.lines) for window in find_windows(html)] == [
        ("chat", [first]), ("chat", [reply]), ("chat", [question]), ("chat", pair),
    ]
    # Не ответ: через два абзаца повествования, после разрыва сцены, не имя в начале
    # строки, склейка через <br>, непойманная реплика следом.
    lead = "Этим и был вызван его диалог по мобильному телефону."
    for html in [
        _chapter(lead, first, "Он ждал ответа.", "Прошла минута.", reply, "Выверт замер."),
        _chapter(lead, first, "Он ждал ответа.", reply, "Выверт замер.").replace(
            "<p>Он ждал ответа.</p>", "<hr/>\n<p>Он ждал ответа.</p>"),
        _chapter(lead, first, "Он ждал ответа.", "Вывод: «Надо уходить».", "Выверт замер."),
        _chapter(lead, first, "Он ждал ответа.", "Баланс: «Отставить».<br/>Выверт: «Понял».", "Выверт замер."),
        _chapter(lead, first, "Он ждал ответа.", reply, "Цзян Лай: «.» (Ничего себе!)"),
    ]:
        assert [(window.kind, window.lines) for window in find_windows(html)] == [("chat", [first])], html
    # Непойманная реплика между чатом и ответом его не обрывает: переписка вокруг уже
    # в рамках.
    series = ["Выверт: «Приговор гарантирован».", "Баланс: «Отставить».", "Выверт: «Понял»."]
    html = _chapter(lead, *series, "Цзян Лай: «.» (Ничего себе!)", reply, "Выверт замер.")
    assert [(window.kind, window.lines) for window in find_windows(html)] == [("chat", series), ("chat", [reply])]


# --- текст абзаца у инлайн-тегов и новые формы строк переписки --------------------------


def test_inline_tags_do_not_put_spaces_into_the_text():
    # «Сын Симург»: «Я <em>знаю</em>. Я тот бедолага…» — в пузыре было «знаю .».
    html = _chapter(
        "Он открыл телефон и прочитал сообщение.",
        "Гарри: «Я <em>знаю</em>. Я тот бедолага».",
        "Видео смотреть <strong>здесь</strong>.",
        "<span>Сло</span><span>во</span> за словом",
        "Строка<br/>вторая",
    )
    assert [paragraph.text for paragraph in sw._paragraphs(html)] == [
        "Он открыл телефон и прочитал сообщение.",
        "Гарри: «Я знаю. Я тот бедолага».",
        "Видео смотреть здесь.",
        "Слово за словом",
        "Строка вторая",
    ]
    assert [window.lines for window in find_windows(html)] == [["Гарри: «Я знаю. Я тот бедолага»."]]


def test_quoted_nicknames_make_a_chat():
    # «Магическая битва. Этот Сукуна слишком добрый», гл. 19: ники в кавычках.
    lines = [
        "«Зовите меня Маки»: Чего случилось?",
        "«Тунец с майонезом»: Кое-кто готовит десерт на кухне!",
        "Нобара: Десерт? Кто готовит?",
        "«Тунец с майонезом»: В чате неудобно объяснять, просто дуйте сюда!",
        "«Зовите меня Маки»: Ладно-ладно, иду… Но если это ерунда, я вам обоим наваляю.",
    ]
    html = _chapter("Телефон завибрировал.", *lines, "Итадори вздохнул.")
    assert [(window.kind, window.lines) for window in find_windows(html)] == [("chat", lines)]
    assert sw.chat_line(lines[0]) == sw.ChatLine("Зовите меня Маки", "Чего случилось?", "nick")
    assert sw.chat_line("«Не ем кинзу»: «Я студентка».") == sw.ChatLine("Не ем кинзу", "Я студентка.", "nick")
    # Кавычки отделяют ник, и он бывает длиннее имени («Бизнес с карточками»).
    assert sw.chat_line("«Красавчик 180 см №1»:…? Это что?") == sw.ChatLine("Красавчик 180 см №1", "…? Это что?", "nick")
    assert sw._speaker_ok(sw.chat_line("«Шестая конференция вот-вот начнется»: Я немного вложила в фонды."))
    # Навыки той же формы — не переписка: ни живой речи, ни повторов.
    skills = [
        "«Поглощение маны»: если раньше оно восстанавливало по 300 единиц маны, то теперь по 500.",
        "«Бледный каскад»: создает три защитные сферы, которые взрываются при контакте.",
        "«Поглощение маны»: время действия увеличено до 20 секунд.",
    ]
    assert all(window.kind != "chat" for window in find_windows(_chapter("Он открыл панель.", *skills, "Закрыл.")))
    # Одна строка с ником в кавычках — не одиночное сообщение: так пишут и навыки.
    html = _chapter("Пришло сообщение.", "«Лавка Сяо Цао»: Я тоже его видела!", "Он усмехнулся.")
    assert all(window.kind != "chat" for window in find_windows(html))


def test_attachments_are_messages_of_their_sender():
    # «Да вы издеваетесь!»: «Гу Чэньси: [Смайлик с поцелуем]»; Kumo desu ka:
    # «Сяобай: [Обеденный_стол. Jpg]. «…»».
    assert sw.chat_line("Ван Ци: [Геолокация]") == sw.ChatLine("Ван Ци", "[Геолокация]", "bare")
    assert sw.chat_line("Mr10tickles: [изображение].") == sw.ChatLine("Mr10tickles", "[изображение].", "bare")
    assert sw.chat_line("Сяобай: [Обеденный_стол. Jpg]. «Я вернулся в замок!»") == sw.ChatLine(
        "Сяобай", "[Обеденный_стол. Jpg] Я вернулся в замок!", "quoted")
    lines = [
        "Гу Чэньси: «Малыш, не обижайся, вот тебе поцелуйчик в награду».",
        "Гу Чэньси: [Смайлик с поцелуем]",
        "Кун Лю: «Я хочу настоящий».",
    ]
    html = _chapter("Немного погодя она тайком отправила Кун Лю сообщение.", *lines, "Гу Чэньси покраснела.")
    assert [(window.kind, window.lines) for window in find_windows(html)] == [("chat", lines)]
    # Скобки без вложения — поле карточки или система, а не сообщение; вложение без
    # имени отправителя — повествование («Да вы издеваетесь!», гл. 324).
    for text in ("Возраст: [19]", "Статус: [УСПЕШНО]", "Под вопросом: [Прожорливость].",
                 "Навыки: [Драконья мощь] [Боевая магия]", "В следующую секунду: [Перевод 1000 юаней].",
                 "Лю Шэн: [Огненный шар]"):
        assert sw.chat_line(text) is None, text


def test_footnote_or_attachment_note_after_a_quoted_message():
    # So I'm an Earth: «Фэй6: «ЭТО КАК ТАК-ТО?!» (1)»; Star Rail: «(Прикреплено фото: …)».
    assert sw.chat_line("Фэй6: «ЭТО КАК ТАК-ТО?!» (1)") == sw.ChatLine("Фэй6", "ЭТО КАК ТАК-ТО?! (1)", "quoted")
    assert sw.chat_line("Стелла: «Юнь Хао не дотянул». (Прикреплено фото: Юнь Хао.jpg)") == sw.ChatLine(
        "Стелла", "Юнь Хао не дотянул. (Прикреплено фото: Юнь Хао.jpg)", "quoted")
    # Номер части в заголовке и пояснение в скобках — не приписка к сообщению
    # («Да вы издеваетесь!», гл. 727: «(смайлик нарисовала Ян Сю…)» — это рассказчик).
    assert sw.chat_line("Глава 659: «Сердце Вечности» (III)") is None
    assert sw.chat_line("Ван Ци: «Готов быть неудачником! ^_^» (смайлик нарисовала Ян Сю)") is None
    assert sw.chat_line("Боевой дух: «Книга Жизни и Смерти» (боевой дух безупречного качества)") is None


def test_channel_or_time_before_the_name():
    # LOL: «[Все] WE 957: «…»»; The Dark Below: «[Пятница; 1530] Шинсо: …».
    assert sw.chat_line("[Всем] AHQ: GG!") == sw.ChatLine("AHQ", "[Всем] GG!", "bare")
    assert sw.chat_line("[Все] EDG Nexus: «Я на ротации».") == sw.ChatLine("EDG Nexus", "[Все] Я на ротации.", "quoted")
    lines = [
        "[Пятница; 1530] Шинсо: Ответить не так уж сложно.",
        "[Пятница; 1603] Шинсо: Ладно, неважно. Можешь поговорить со мной, когда будешь готов.",
    ]
    html = _chapter("Телефон снова пискнул.", *lines, "Он убрал телефон.")
    assert [(window.kind, window.lines) for window in find_windows(html)] == [("chat", lines)]
    assert not sw._bracketed(lines[0]) and sw._bracketed("[Он видит вас. Немедленно отступайте!]")
    # Метка без слова канала или времени — не канал: «[X] План: …», «[Пустота] Жанр: …».
    assert sw.chat_line("[X] План: Уклончивые ответы.") is None
    assert sw.chat_line("[12:30] [Система]: Перезагрузка.") is None
    assert sw.chat_line("[Пустота] Жанр: Современность / Драма (8 игроков)") is None


def test_number_at_the_end_of_a_name():
    # Star Rail: «Март 7: «…»».
    lines = [
        "Март 7: «Что случилось? Что-то хорошее?»",
        "Стелла: «Юнь Хао всё, не дотянул». (Прикреплено фото: Юнь Хао.jpg)",
        "Вельт: «Спит стоя? Поразительный навык».",
        "Март 7: «Ха-ха, я тоже хочу так уметь!»",
    ]
    html = _chapter("В групповом чате экипажа было оживленно.", *lines, "Юнь Хао криво усмехнулся.")
    assert [(window.kind, window.lines) for window in find_windows(html)] == [("chat", lines)]
    # Заголовок с номером и время — не имя. Номер в имени — только у реплики в кавычках:
    # «[Пункт 1: …]», «[Способ 2: …]» — нумерованный список системы.
    assert sw._looks_like_name("Март 7", numbered=True) and not sw._looks_like_name("Март 7")
    assert not sw._looks_like_name("Глава 7", numbered=True) and not sw._looks_like_name("Этаж 5", numbered=True)
    assert sw.chat_line("В 5:30 утра он проснулся.") is None
    items = [
        "[Пункт 1: Любые эмоции теперь могут быть преобразованы в очки эмоций.]",
        "[Пункт 2: Количество очков для прорыва уменьшено вдвое.]",
        "[Пункт 3: Обновлена лотерея высочайшего ранга.]",
    ]
    assert all(window.kind != "chat" for window in find_windows(_chapter("Система обновилась.", *items, "Он моргнул.")))
    items = ["Пункт 1: Вы получите награду за каждый день!", "Пункт 2: Вы потеряете очки за пропуск!"]
    html = _chapter("Пришло системное сообщение.", *items, "Он моргнул.")
    assert all(window.kind != "chat" for window in find_windows(html))


def test_numbered_speakers_of_one_family_are_known_together():
    # So I'm an Earth, гл. 57: «Фэй1: «ШО?!»» … «Фэй9: «ЧТО?!»» — Фэй7 в других
    # переписках книги не пишет, но её сёстры знакомы.
    lines = ["Фэй1: «ШО?!»", "Фэй2: «ЧИВО?!»", "Фэй7: «Шэньмэ?!»"]
    assert sw.chat_verdict(lines, frozenset({"Фэй1", "Фэй2"})) == "chat"
    assert sw.chat_verdict(["Ли: «Кто это?»", "Ван: «Не знаю»."], frozenset({"Ли"})) == "weak"


def test_reviewer_replies_are_not_a_status_card():
    # Fairy Tail Xenoverse, гл. 36: ответы на отзывы; ник рецензента размечен кусками.
    html = _chapter(
        "В общем.",
        "Elvizfonz23: Спасибо!",
        "<strong>Aadidoestfp:</strong> Не, бро, я к такому пока не готов.",
        "<strong>Guest Reviewer «</strong><strong>fairy</strong><span> saiyan»: </span>М-да.",
        "Глава начинается.",
    )
    assert find_windows(html) == []


def test_short_exchange_after_words_about_messages_is_a_chat():
    # Predatory, гл. 12: пара без повторов, её собеседники в книге больше не пишут —
    # но абзацем выше «отправила ответ». Так же узнаётся и одно сообщение.
    lines = ["FlippinMad: Я не знаю, кто это.", "Mr10tickles: [изображение]."]
    html = _chapter("Наконец она отправила ответ со своего основного аккаунта.", *lines, "Глаза Мэдисон округлились.")
    assert [(window.kind, window.lines) for window in find_windows(html)] == [("chat", lines)]
    assert find_windows(_chapter("Мэдисон задумалась.", *lines, "Глаза Мэдисон округлились.")) == []


def test_messaging_words_for_an_exchange_come_from_narration():
    # «Бизнес», гл. 336: заголовок-цитата со словом «QQ» перепиской не делает.
    heads = [
        "«Минутка ликбеза: сколько времени проходит от иска до первого заседания?»",
        "«QQ вновь заявляет: продукт является полностью самостоятельной разработкой»",
    ]
    html = _chapter("«Суд района Чаоян принял к рассмотрению иск 99 к QQ о нарушении прав!»", *heads, "Все запаслись попкорном.")
    assert all(window.kind != "chat" for window in find_windows(html))


def test_tail_of_an_exchange_is_not_accepted_alone():
    # «Состояние в десятки триллионов!», гл. 299: поздравления звёзд; знакомы только двое
    # последних — хвост серии отдельно не оформляется.
    lines = [
        "【Чэнь Даоминь V】: Поздравляю госпожу Сюй Лу с днем свадьбы!",
        "【Чэнь Кайгэ V】: Поздравляю госпожу Сюй Лу!",
        "【Ма Хуатэн V】: Прошу прощения, не смог вырваться!",
        "【Ван Цзяньлинь V】: Ха-ха-ха! Поздравляю племянницу!",
    ]

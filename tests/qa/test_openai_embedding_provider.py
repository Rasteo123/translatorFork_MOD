...
def test_openai_posts_one_ordered_batch_and_restores_index_order():
    ...
    result = _embed(
        OpenAICompatibleEmbeddingProvider("https://example.test/v1/chat/completions", "secret", _factory(session), 30),
        _request("a", "b"),
    )
...
def test_openai_normalizes_root_and_preserves_safe_deployment_prefix(base_url, expected):
    ...
    _embed(OpenAICompatibleEmbeddingProvider(base_url, "", _factory(session), 1), _request("x"))
...
def test_openai_rejects_unsafe_endpoint_without_echoing_it(base_url):
    ...
    with pytest.raises(Exception) as captured:
        OpenAICompatibleEmbeddingProvider(base_url, "key", _factory(_Session()), 1)
...
def test_openai_maps_http_status_before_json_and_closes_contexts(status, retryable):
    ...
    with pytest.raises(EmbeddingHttpError) as captured:
        _embed(OpenAICompatibleEmbeddingProvider("https://example.test", "secret", _factory(session), 1), _request("text"))
...
def test_openai_treats_sentinel_key_as_no_authentication():
    ...
    session = _Session(_Response(payload={"data": [{"index": 0, "embedding": [1.0, 0.0]}]}))
    _embed(OpenAICompatibleEmbeddingProvider("http://localhost:1234", "  __local__ ", _factory(session), 1), _request("x"))
    assert session.request.headers == {"Content-Type": "application/json"}
...
def test_openai_omits_dimensions_when_the_embedding_request_does_not_set_them():
    ...
    session = _Session(_Response(payload={"data": [{"index": 0, "embedding": [1.0, 0.0]}]}))
    _embed(OpenAICompatibleEmbeddingProvider("https://example.test", "", _factory(session), 1), _request("x", dimensions=None))
...
def test_openai_maps_timeout_and_invalid_json_to_typed_safe_errors():
    ...
    with pytest.raises(EmbeddingTransportError) as transport:
        _embed(OpenAICompatibleEmbeddingProvider("https://example.test", "key", _factory(session), 1), _request("secret request"))
...
def test_openai_rejects_duplicate_missing_boolean_or_ragged_response_rows(payload):
    ...
    with pytest.raises(EmbeddingResponseError):
        _embed(
            OpenAICompatibleEmbeddingProvider("https://example.test", "key", _factory(_Session(_Response(payload=payload))), 1),
            _request("a", "b"),
        )
...
def test_openai_propagates_cancellation():
    ...
    with pytest.raises(asyncio.CancelledError):
        _embed(
            OpenAICompatibleEmbeddingProvider("https://example.test", "key", _factory(_Session(post_error=asyncio.CancelledError())), 1),
            _request("x"),
        )

"""Install the optional Slovnet weights explicitly, verifiably, and reversibly."""
...
class SlovnetModelManager:
    ...
    def __init__(self, root: Path | str, manifest: SlovnetManifest, downloader=None) -> None:
        if not isinstance(manifest, SlovnetManifest):
            raise TypeError("manifest must be a SlovnetManifest")
        self.root = Path(root)
    ...
    def status(self) -> SlovnetModelStatus:
        ...
        if not directory.is_dir():
            return SlovnetModelStatus("missing")
        installed = 0
        for item in self.manifest.files:
            path = directory / item.name
            if not path.is_file():
                return SlovnetModelStatus(
        ...
        return SlovnetModelStatus("ready", self.manifest.version, installed)

    async def install(self, progress=None, cancellation=None) -> SlovnetModelStatus:
        """Download and verify every file, then swap the directory in atomically."""
        if self._downloader is None:
            raise RussianNlpUnavailable("slovnet_downloader_missing")
        if self.status().state == "ready":
            return self.status()
        self.root.mkdir(parents=True, exist_ok=True)
        staging = Path(tempfile.mkdtemp(prefix=".slovnet-", dir=str(self.root)))
        downloaded = 0
        try:
            for item in self.manifest.files:
                ...
                if not isinstance(data, (bytes, bytearray)):
                    raise RussianNlpUnavailable("slovnet_download_invalid")
                digest = hashlib.sha256(data).hexdigest()
                if digest != item.sha256:
                    raise RussianNlpUnavailable(f"slovnet_hash_mismatch:{item.name}")

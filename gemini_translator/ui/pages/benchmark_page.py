"""PromptBenchmarkPage — prompt/model benchmark editor and runner as an embeddable ShellPage."""
...
class PromptBenchmarkPage(ShellPage):
    ...
    page_title = "Бенчмарк промптов и моделей"
    ...
    def _build_cases_tab(self):
        ...
        self.case_glossary_table.setAlternatingRowColors(True)
    ...
    def can_leave(self) -> bool:
        if self.worker is not None and self.worker.isRunning():
            QtWidgets.QMessageBox.warning(
                self, "Бенчмарк",
                "Дождитесь завершения текущего запуска перед выходом.",
            )
            return False
        return True

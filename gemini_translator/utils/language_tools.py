...
def _get_opencc_converter(config_name):
    if not OPENCC_AVAILABLE:
        return None

    converter = _OPENCC_CONVERTERS.get(config_name)
    if converter is None:
        try:
            converter = OpenCC(config_name)
        except Exception:
            return None
        _OPENCC_CONVERTERS[config_name] = converter
    return converter


def get_chinese_script_variants(text):
    ...
    for config_name in ("t2s", "s2t"):
        converter = _get_opencc_converter(config_name)
        if converter is None:
            continue
        try:
...
class SmartGlossaryFilter:
    ...
    def _validate_chinese_candidates(self, candidates, normalized_text):
        ...
        for term in candidates:
            ...
            for script_variant in get_chinese_script_variants(term):
                # Мы должны проверить каждую часть многословного термина
                clean_term_str = self._universal_cleaner_re.sub(' ', self._normalize_text(script_variant)).strip()
    ...
    def process_html(self, html_content: str) -> str:
        ...
        for original, data in found_glossary_subset.items():
            ...
            if not rus:
                continue
            for script_variant in get_chinese_script_variants(original):
                replacements[script_variant] = rus

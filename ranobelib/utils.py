...
def format_timedelta(td: timedelta) -> str:
    ...
    Adapts the timedelta interface (and this project's "negative -> —"
    convention for an elapsed/unknown ETA) onto the shared
    gemini_translator.utils.text.format_duration, which every other
    duration display in the project also goes through.

    The import is deliberately local (not at module level): this is a
    leaf module used by lightweight ranobelib consumers (models, parsers,
    dialogs, headless epub parsing, tests) that never call this function,
    and gemini_translator.utils.text pulls in lxml/bs4/requests plus the
    translator's api/config module at import time. A module-level import
    would make every such consumer pay for that stack just to import this
    file (measured: ranobelib/utils.py import time 14ms -> 132ms). Once
    something does call format_timedelta, gemini_translator.utils.text is
    already cached in sys.modules, so this costs nothing on repeated calls.
    """
    from gemini_translator.utils.text import format_duration

    return format_duration(int(td.total_seconds()), unit_spacing="", dash_on_negative=True)
